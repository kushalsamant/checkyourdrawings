...
def save_overlay_pdf(
    ...
    *,
    ...
    """Write a one-page PDF containing the coordination overlay."""
    if raster_dpi <= 0:
        raise ValueError("raster_dpi must be greater than zero.")
    if page_width_pt <= 0 or page_height_pt <= 0:
        raise ValueError("page dimensions must be greater than zero.")
    if layout not in {"crop", "full_sheet"}:
        raise ValueError("layout must be 'crop' or 'full_sheet'.")
    ...
    try:
        if layout == "crop":
            page_width_pt = overlay_width / raster_dpi * 72.0
            page_height_pt = overlay_height / raster_dpi * 72.0
