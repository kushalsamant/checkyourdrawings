...
def active_job_limit_detail(limit: int) -> str:
    if limit <= 1:
        return "Another comparison is in progress. Wait for it to finish before starting a new one."
    return f"You have reached the limit of {limit} active comparisons. Wait for one to finish before starting another."
